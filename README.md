...
### ✅ Задание №6. Поиск комбинации чисел с заданной суммой

**Описание**

Реализована программа для поиска набора чисел из заданной последовательности, сумма которых равна или максимально приближена к указанному пользователем значению.

Исходная последовательность:

```text
11, 23, 45, 120, 38, 155, 86, 181, 65
```

Алгоритм выполняет перебор всех возможных комбинаций элементов последовательности и выбирает комбинацию с минимальным отклонением от целевой суммы.

Для перебора комбинаций используется метод битовых масок. Каждый элемент последовательности соответствует одному биту маски:

* `1` — элемент входит в текущую комбинацию;
* `0` — элемент не используется.
...
Для каждой комбинации вычисляется сумма выбранных элементов и сравнивается её отклонение от заданного значения. Если найденное отклонение меньше текущего минимального, комбинация сохраняется как лучший результат.
...
```text
Создать начальные значения:

bestDifference = максимальное значение
bestNumbers = пустой массив


Для каждой битовой маски:

    определить выбранные элементы

    вычислить сумму выбранных элементов
    ...
    если отклонение меньше лучшего:

        сохранить новую комбинацию
```

**Пример работы алгоритма**

Для целевой суммы `S = 200` возможные комбинации сравниваются между собой:

```text
[11, 23, 45, 120] → сумма 199 → отклонение 1

[120, 86] → сумма 206 → отклонение 6

[181, 23] → сумма 204 → отклонение 4
```

В качестве результата выбирается комбинация:
